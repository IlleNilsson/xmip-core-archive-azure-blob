//! The layout of one item in the container: the name its bytes are stored
//! under, the name its metadata sits beside them under, the text the
//! metadata is kept as, and the checksum the receipt carries.
//!
//! `<prefix>/<data_type>/<identifier>` holds the bytes and the same name
//! with `.meta` appended holds the metadata, so an operator listing the
//! prefix sees the archive laid out by type and can open either blob
//! with any Blob tool. The data type is made slash-free so the name splits
...
/// What follows an item's name to name the blob holding its metadata.
pub const META_SUFFIX: &str = ".meta";

/// The name for an item's bytes: `<prefix>/<data_type>/<identifier>`, or
...
/// The name for the metadata beside the bytes at `key`.
#[must_use]
pub fn meta_key(key: &str) -> String {
    format!("{key}{META_SUFFIX}")
}

/// The data type and identifier a name under `prefix` carries, or `None` when
/// the name is not laid out that way.
#[must_use]
...
/// The metadata pairs as the one text the `.meta` blob holds.
#[must_use]
...
/// `text_plain`, so the name still splits into type and identifier.
